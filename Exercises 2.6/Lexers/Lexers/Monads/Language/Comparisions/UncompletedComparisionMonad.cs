...
public record UncompletedComparisionMonad(string Lexeme, Location Location) : UncompletedLexemeMonad(Lexeme, Location)
{
    public override LexemeMonad Append(char character, Position position) => character switch
    {
        '=' => new CompletedLexemeMonad(new ComparisionToken(Lexeme + character, Location.EndAt(position)), null),
        _ => new CompletedLexemeMonad(new ComparisionToken(Lexeme, Location), RootMonad.Remain(character, position))
    };
